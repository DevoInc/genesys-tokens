...
describe('formats', () => {
  describe('html', () => {
    describe('flatten', () => {
      const cases: [
        string,
        { tokens: { [key: string]: string }; types: string[] },
        string
      ][] = [
        [
          'two properties',
          {
            tokens: {
              ...
              'cmp-breakpoints-mediaQuery-sm': '576px',
            },
            types: ['alias', 'cmp'],
          },
          `
...
<body>
  ...
  <script>
    const tokensArr = {"alias-breakpoints-mediaQuery-xs":"575px","alias-breakpoints-mediaQuery-sm":"576px","cmp-breakpoints-mediaQuery-sm":"576px"};
  ...
  <script>
    ...
    setListContent();
      ...
      it.each(cases)('%s', (_title, { tokens, types }, expected) => {
        console.log(template({ tokens: JSON.stringify(tokens), types }));
        expect(template({ tokens: JSON.stringify(tokens), types })).toEqual(
          expected
        );
      });
    });
